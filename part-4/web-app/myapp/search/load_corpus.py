...
from myapp.core.utils import load_json_file, load_csv_file
from myapp.search.objects import Document

_corpus = {}


def load_clicks_data(path):
    data = load_csv_file(path)
    data_dict = dict([])
    for key in data:
        data_dict[int(key)] = int(data[key][0])
    return data_dict 

def load_queries_data(path):
    data = load_csv_file(path)
    data_dict = dict([])
    for key in data:
        try:
            data_dict[key] = int(data[key][0])
        except:
            continue
    return data_dict

def load_numbers_data(path):
    data = load_csv_file(path)
    data_dict = dict([])
    for key in data:
        try:
            ...
            data_dict[key] = float(data[key][0])
    return data_dict  
...
def _load_corpus_as_dataframe(path):
    ...
    corpus["Url"] = "https://twitter.com/" + corpus["Username"] +"/status/" + corpus["Id"].map(str)
